...
namespace SteamDlcShopping
{
    ...
    {
        private readonly long SteamId3;
        internal SortedDictionary<string, List<string>> collections;

        public FrmCollections(long steamId3)
        {
            InitializeComponent();
        ...
        private void FrmCollections_Load(object sender, EventArgs e)
        {
            ...
            //Reverse file structure from games in collections to collections with games
            foreach (KeyValuePair<string, VdfFileNode> app in appsNode.NodeArray)
            {
                if (!app.Value.NodeArray.ContainsKey("tags"))
                {
                    continue;
                }

                VdfFileNode tags = app.Value.NodeArray["tags"];

                foreach (KeyValuePair<string, VdfFileNode> collection in tags.NodeArray)
                {
                    if (!collections.ContainsKey(collection.Value.NodeString))
                    {
                        collections.Add(collection.Value.NodeString, new List<string>());
                    }

                    collections[collection.Value.NodeString].Add(app.Key);
                }
            }

            //Populate listview with categories
            foreach (KeyValuePair<string, List<string>> collection in collections)
            {
                ...
                lsvCollections.Items.Add(item);
            }

            Location = new Point(Owner.Location.X - 225, Owner.Location.Y + 148);
        }
    }
    ...
    public class VdfFileNode
    {
        ...
        private static string ReadBin_GetStringToken(BinaryReader reader, long streamLength = -1)
        {
            ...
            if (!stringDone)
            {
                if (endOfStream)
                {
                    throw new ParseException("TextVdfFile_UnexpectedEOF");
                }
            }
        ...
        public static VdfFileNode LoadFromText(StreamReader stream, bool useFirstAsRoot = false)
        {
            ...
            while (!stream.EndOfStream)
            {
                ...
                char nextChar = (char)stream.Read(); //Get key
                string key;
                ...
                nextChar = (char)stream.Read(); //Get value
                VdfFileNode newNode;
